...
public class CorrectUtils {
    ...
    public String[] correctTypeV2(String[] temp ) throws SQLException {
        ...
        // out解的容器  (可能造成题型比例失衡的解集)
        Set<String> outMore = getOutMoreTypeV2(batchItemList,tf1,tf2,tf3,tf4);
        Set<String> outLess = getOutLessTypeV2(batchItemList,tf1,tf2,tf3,tf4);
        ...
        // 画横线的地方均需考虑，是否会发生数据的变化
        if(outMore.size()==0 && outLess.size()>0){
            batchItemList = correctTypeLessV2(outLess,jdbcUtils,batchItemList,tf1,tf2,tf3,tf4);
        }


        //**************  3.3 outLess无  outMore有值   *************
        if(outMore.size()>0 && outLess.size()==0){
            batchItemList = correctTypeMoreV2(outMore,jdbcUtils,batchItemList,tf1,tf2,tf3,tf4);
        }
     ...
     *
     */
    private Set<String> getOutMoreTypeV2(ArrayList<String> batchItemList, int tf1, int tf2, int tf3, int tf4){
     ...
     *
     */
    private Set<String> getOutLessTypeV2(ArrayList<String> batchItemList, int tf1, int tf2, int tf3, int tf4){
     ...
     *
     */
    private ArrayList<String> correctTypeLessV2(Set<String> outLess, JDBCUtils4 jdbcUtils, ArrayList<String> batchItemList, int tf1, int tf2, int tf3, int tf4) throws SQLException {
        ...
        for (int i = 0; i < outList.size(); i++) {
            ...
            String p1 = " and( p1 = " + (outList.get(i).split(":")[2]!="0.0"?1:0) ;
            String p2 = " and p2 = " +  (outList.get(i).split(":")[3]!="0.0"?1:0);
            String p3 = " and p3 = " +  (outList.get(i).split(":")[4]!="0.0"?1:0);
            String p4 = " and p4 = " +  (outList.get(i).split(":")[5]!="0.0"?1:0);
            String p5 = " and p5 = " +  (outList.get(i).split(":")[6]!="0.0"?1:0) + " ) ";

            // 获取完全吻合的解  题型+属性
            String sqlPerfect = sql + (p1 + p2 + p3 + p4 + p5);
            // FIXME 最好通过sql去打乱顺序inList2
            ArrayList<String> inList2 = jdbcUtils.selectBySqlV2(sqlPerfect);
            ...
            if(inList2.size()>0){
                ...
                ArrayList<String> inList = jdbcUtils.selectBySqlV2(sql);
     ...
     *
     */
    public ArrayList<String> correctTypeMoreV2(Set<String> outMore,JDBCUtils4 jdbcUtils,ArrayList<String> batchItemList,int tf1,int tf2,int tf3,int tf4) throws SQLException {
     ...
     *
     */
    public String[] correctAttributeV3(String[] temp) throws SQLException {
        ...
        // 获取个体的题型指标信息
        String attributeFlag = getAttributeFlagV2(itemSet);
        ...
        //取出属性比例过多的集合的并集
        //Set<String> outMore = getOutMoreAttrV2(batchItemList,af1,af2,af3,af4,af5,af6,af7);
        Set<String> outMore =  new HashSet<String>();;

        //取出属性比例不足的集合的并集
        //Set<String> outLess = getOutLessAttrV2(batchItemList,af1,af2,af3,af4,af5,af6,af7);
        Set<String> outLess =  new HashSet<String>();;
        ...
        if(outMore.size()==0 && outLess.size()>0){
            ...
            // FIXME 待完成
            //batchItemList = correctAttributeLessV2(outLess,jdbcUtils,batchItemList,af1,af2,af3,af4,af5);
        ...
        if(outMore.size()>0 && outLess.size()==0){
            ...
            // FIXME 待完成
            // batchItemList = correctAttributeMoreV2(outMore,jdbcUtils,batchItemList,af1,af2,af3,af4,af5);
...
//        int af7 = Integer.parseInt(attributeFlag.split(",")[6]);
     ...
     *
     */
    private String getAttributeFlagV2(HashSet<String> itemSet){
        ...
        for (String s:itemSet) {
            ...
            if (!"0.0".equals(split[7])) {
                attributeNum6 += 1;
            }
            if (!"0.0".equals(split[8])) {
                attributeNum7 += 1;
            }
        ...
        //属性比例
        double attributeRatio1 = attributeNum1/30.0;
        double attributeRatio2 = attributeNum2/30.0;
        double attributeRatio3 = attributeNum3/30.0;
        double attributeRatio4 = attributeNum4/30.0;
        double attributeRatio5 = attributeNum5/30.0;
        double attributeRatio6 = attributeNum6/30.0;
        double attributeRatio7 = attributeNum7/30.0;


        int af1 ;
        if(attributeRatio1>=0.1 && attributeRatio1<=0.3){
            af1 = 0;
        }else if(attributeRatio1<0.1){
            af1 = -1;
        }else {
            af1 = 1;
        }
        ...
        int af3 ;
        if(attributeRatio3>=0.2 && attributeRatio3<=0.4){
            af3 = 0;
        }else if(attributeRatio3<0.2){
            af3 = -1;
        }else {
            af3 = 1;
        }
        ...
        int af5 ;
        if(attributeRatio5>=0.2 && attributeRatio5<=0.4){
            af5 = 0;
        }else if(attributeRatio5<0.2){
            af5 = -1;
        }else {
            af5 = 1;
        }
        ...
        int af6 ;
        if(attributeRatio6>=0.1 && attributeRatio6<=0.3){
            af6 = 0;
        }else if(attributeRatio6<0.1){
            af6 = -1;
        }else {
            af6 = 1;
        }
        ...
        int af7 ;
        if(attributeRatio7>=0.1 && attributeRatio7<=0.3){
            af7 = 0;
        }else if(attributeRatio5<0.1){
            af7 = -1;
        }else {
        ...
        //输出 attributeFlag
        String attributeFlag = af1+","+af2+","+af3+","+af4+","+af5+","+af7+","+af7;
        //System.out.println("目前属性占比情况： attributeFlag:("+attributeFlag+")");
        return attributeFlag;
     ...
     *
     */
    private Set<String> getOutMoreAttrV2(ArrayList<String> bachItemList, int af1, int af2, int af3, int af4, int af5, int af6, int af7){
     ...
     *
     */
    private Set<String> getOutLessAttrV2(ArrayList<String> bachItemList, int af1, int af2, int af3, int af4, int af5, int af6, int af7){
        ...
        if(af1==-1){
            for (String aBachItemList : bachItemList) {
                String[] split = aBachItemList.split(":");
                if (!"0.0".equals(split[2])) {
                    outLess.add(aBachItemList);
                }
            }
     ...
     *
     */
    private ArrayList<String> correctAttributeLessV2(Set<String> outLess, JDBCUtils4 jdbcUtils, ArrayList<String> bachItemList, int af1, int af2, int af3, int af4, int af5) throws SQLException {
        ...
        for (int i = 0; i < outList.size(); i++) {
            ...
            for (int j = 0; j < inListRe.size(); j++) {
                ...
                // FIXME 属性校验待完成
                //b = attributeCheckV2(tmp,outList.get(i),inListRe.get(j));
     ...
     *
     */
    public ArrayList<String> correctAttributeMoreV2(Set<String> outMore,JDBCUtils4 jdbcUtils,ArrayList<String> bachItemList,int af1,int af2,int af3,int af4,int af5) throws SQLException {
     ...
     *
     */
    private Boolean attributeCheckV2(ArrayList<String> tmp,String s, String s1) {
